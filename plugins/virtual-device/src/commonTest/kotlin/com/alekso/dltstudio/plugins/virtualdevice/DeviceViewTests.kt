...
class DeviceViewTests {
    ...
    fun `Test DeviceView parsing`() {
        ...
        val expectedText = listOf(
            RectView(Rect(2298f, 22f, 2835f, 709f), id = "app:id/theme_container"),
            RectView(Rect(0f, 0f, 1f, 1f), id = "app:id/focus_parking_view"),
        )
        val parsed = DeviceViewParser().parse(text)
        Assert.assertTrue(
            "\n$parsed\n!=\n$expectedText",
            parsed == expectedText
        )
    ...
    fun `Test Touch region parsing`() {
        ...
        val expectedText = listOf(
            RectView(Rect(1132f, 40f, 1132f + 240f, 40f + 115)),
            PointerView(1132f, 40f),
        )
        val parsed = DeviceViewParser().parse(text)
        Assert.assertTrue(
            "\n$parsed\n!=\n$expectedText",
    ...
    fun `Test DeviceView parsing curly brackets`() {
        ...
        val expectedText = listOf(
            RectView(Rect(576f, 0f, 696f, 120f), id = "app:id/menuIcon"),
            RectView(Rect(0f, 0f, 1f, 1f), id = "app:id/focus_parking_view"),
        )
        val parsed = DeviceViewParser().parse(text)
        Assert.assertTrue(
    ...
    fun `Test DeviceView parsing without id`() {
        ...
        val expectedText = listOf(RectView(Rect(0f, 10f, 10f, 20f)))
        val parsed = DeviceViewParser().parse(text)
        Assert.assertTrue(
            "\n$parsed\n!=\n$expectedText",
            parsed == expectedText
    ...
    fun `Test DeviceView parsing negative width`() {
        ...
        val expectedText = listOf(RectView(Rect(384f, 0f, 396f, 0f)))
        val parsed = DeviceViewParser().parse(text)
        Assert.assertTrue(
            "\n$parsed\n!=\n$expectedText",
            parsed == expectedText

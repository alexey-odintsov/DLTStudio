...
import alexey.odintsov.dltstudio.plugins.diagramtimeline.filters.ExtractorChecker
...
fun EditTimelineFilterExtractPanel(viewModel: EditTimelineFilterViewModel) {
        ...
        Column(Modifier.weight(1f)) {
            ...
            when (viewModel.extractorType) {
                EntriesExtractor.ExtractionType.NamedGroupsOneEntry -> {
                    ...
                    Column {
                        DiagramType.entries.first { it.name == viewModel.diagramType }.params.forEach { (_, param) ->
                            Text(
    ...
    // Regex check
    HorizontalDivider(modifier = Modifier.padding(top = 16.dp, bottom = 8.dp).fillMaxWidth().height(1.dp))
    Row(verticalAlignment = Alignment.CenterVertically) {
